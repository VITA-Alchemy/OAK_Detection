...
class Detector(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.export_onnx:
            ...
            #opencv格式
            t1 = torch.cat((out_reg_2, out_obj_2, out_cls_2), 1).permute(0, 2, 3, 1)
            a = torch.reshape(t1,(-1,95))
            t2 = torch.cat((out_reg_3, out_obj_3, out_cls_3), 1).permute(0, 2, 3, 1)
            b = torch.reshape(t2,(-1,95))
